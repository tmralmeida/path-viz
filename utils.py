...
class AnimTraj():
    ...
    def __sample_ids(self):
        un_ts = self.df["time [ms]"].unique() # get different time stamples
        ...
        ts_ds = self.df[self.df["time [ms]"] == ts] # getting all trajectories related to this timestamp
        ...
        p_ids = random.sample(ts_ds.person_id.tolist(), self.n_traj)
        print(f"Plotting trajectories from {p_ids}...")
        return p_ids
    
    
    def __get_axeslim(self, p_df, threshold = 500):
        x_min = self.df["x[mm]"].min() - threshold
        y_min = self.df["y[mm]"].min() - threshold 
        x_max = self.df["x[mm]"].max() + threshold 
        y_max = self.df["y[mm]"].max() + threshold
        return (x_min, y_min, x_max, y_max)
    
    
    def __get_maxtrajinfo(self, df):
        return df.person_id.value_counts().max(), df.person_id.value_counts().idxmax()
    ...
    def __pre_proctrajs(self):
        ...
        p_df = self.df.loc[self.df["person_id"].isin(p_ids)] # df with the target trajectories
        grouped = p_df.groupby(p_df.person_id) # target groupped 
        
        coords = []
        for i in range(self.n_traj):
            p = grouped.get_group(p_ids[i])
            coords.append(p[["x[mm]", "y[mm]", "z[mm]"]].values)
    ...
    def plot(self, draw_path = True):
        ...
        p_df = self.df.loc[self.df["person_id"].isin(p_ids)] # redundant computation
